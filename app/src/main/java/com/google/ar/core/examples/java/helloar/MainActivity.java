...
import com.google.ar.core.examples.java.services.GazeTrackerManager;
import com.google.ar.core.examples.java.services.ManagedCamera;
import com.google.ar.core.examples.java.view.CalibrationViewer;
import com.google.ar.core.examples.java.view.PointView;
...
public class MainActivity extends AppCompatActivity implements SampleRender.Renderer {
  ...
  private float LastXGaze=0, LastYGaze=0;
  private float xGaze, yGaze;
  private boolean flagPlacement = false;
  ...
  // Gaze Coordinate Filter
  private boolean isUseGazeFilter = true;

  // Multiple Camera
  //ManagedCamera camera0 = null; //Rear Camera
  ManagedCamera camera1 = null; //Front Camera
  TextureView view_finder_0;

  //End GazeTracking Stuff
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    initView();
    checkPermission();
    boolean asked = askAllRequiredPermissions();
    if(!asked){
  ...
  public void onStart(){
    super.onStart();
    if(view_finder_0.isAvailable()){
      gazeTrackerManager.setCameraPreview(view_finder_0);
    }
  ...
  public void onSurfaceCreated(SampleRender render) {
    ...
    try {
      ...
      // Virtual object to render (ARCore campoDiCorvi)
      virtualObjectAlbedoTextureCampoDiCorvi =
          Texture.createFromAsset(
              render,
              "models/campoDiCorvi.png",
              Texture.WrapMode.CLAMP_TO_EDGE,
              Texture.ColorFormat.SRGB);
      virtualObjectAlbedoInstantPlacementTextureCampoDiCorvi =
          Texture.createFromAsset(
              render,
              "models/campoDiCorvi.png",
              Texture.WrapMode.CLAMP_TO_EDGE,
      ...
      virtualObjectShaderCampoDiCorvi =
              Shader.createFromAssets(
                      ...
                      .setTexture("u_AlbedoTexture", virtualObjectAlbedoTextureCampoDiCorvi)
  ...
  private void handleLook(Frame frame, Camera camera) {
  ...
  // Uncomment to use the InstantPlacementSettingsMenu
  /*
  ...
  */
  ...
  private void initView() {
    ...
    view_finder_0 = findViewById(R.id.preview);
  ...
  private boolean askAllRequiredPermissions() {
    int camera_permission_status = ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA);
    if(camera_permission_status != PackageManager.PERMISSION_GRANTED) {
  ...
  public void initCamera() {
    ...
    try {
      if(manager.getCameraIdList().length < 1) {
        Log.i(TAG, "ERRORE INITCAMERA");
      }
      String[] cameraIdList = manager.getCameraIdList();
      //camera0 = new ManagedCamera(cameraIdList[0], "Camera${cameraIdList[0]}}", view_finder_0);
      if (manager.getCameraIdList().length >= 2 ) {
        Log.i(TAG, "CREO CAMERA1");
        camera1 = new ManagedCamera(cameraIdList[1],"Camera${cameraIdList[1]}}", view_finder_0);
      }
  ...
  private void allPermissionGranted() {
    initCamera();
    //camera0.isPreviewing = true;
    camera1.isPreviewing = true;
  }
  ...
  private void checkPermission(boolean isGranted){
    if(isGranted) {
      ...
      showToast("not granted permission", true);
      finish();
    }
  }
  ...
  private void processOnGaze(GazeInfo gazeInfo) {
    if (gazeInfo.trackingState == camp.visual.gazetracker.state.TrackingState.SUCCESS) {
      hideTrackingWarning();
  ...
  private TextureView.SurfaceTextureListener surfaceTextureListener = new TextureView.SurfaceTextureListener() {
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
      ...
      // Uncomment this if you want to show your rear camera
